...
static UniValue validateaddress(const Config &config,
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Bitcoin Cash address to validate"},
                }}
                .ToString() +
            "\nResult:\n"
            "{\n"
            ...
            "signals support for Cash Tokens\n"
            "}\n"
            "\nExamples:\n" +
    ...
    bool tokenAware{};
    CTxDestination dest = DecodeDestination(request.params[0].get_str(), config.GetChainParams(), &tokenAware);
    ...
    if (isValid) {
        ret.emplace_back("address", EncodeDestination(dest, config, tokenAware));

        CScript scriptPubKey = GetScriptForDestination(dest);
        ret.emplace_back("scriptPubKey", HexStr(scriptPubKey));

        DescribeAddress(dest, ret); // isscript

        ret.emplace_back("istokenaware", tokenAware);
    }
    return ret;
}
    ...
    if (request.fHelp || request.params.size() != 3) {
        throw std::runtime_error(
            RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Bitcoin Cash address to use for the signature."},
