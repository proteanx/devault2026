...
#include <qt/dvtui.h>
#include <random>
#include <qt/startoptionssort.h>
#include <qt/forms/ui_startoptionssort.h>
...
StartOptionsSort::StartOptionsSort(std::vector<std::string> Words, int rows,
    ...
    for (int k = 0; k < 6; k++) {
        ...
        itemListWidget->setStyleSheet(
            "QWidget{font-size:18px; font-style:bold; color:#fff;}");
        itemListWidget->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
        labelsList.push_back(itemListWidget);
        ui->gridLayoutRevealed->addWidget(itemListWidget, 2, k,
                                          Qt::AlignCenter);
    }
